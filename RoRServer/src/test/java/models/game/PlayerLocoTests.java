...
public class PlayerLocoTests {
	 ...
	 */
	/*
	 * es müssten die Rails erstellt werden um keine NullPointerExceptions zu
	 * bekommen
	 * 
	 * @Test public void LocoSendsUpdateLocoMessage() { // Square, auf das die Loco
	 * f�hrt int squarePosXNext = 0; int squarePosYNext = 0; // Square, auf dem die
	 * Loco steht int squarePosXLoco = 0; int squarePosYLoco = 1; int squarePosXCart
	 * = 0; int squarePosYCart = 2; Compass node1 = Compass.NORTH; Compass node2 =
	 * Compass.SOUTH; List<Compass> directions = new ArrayList<>();
	 * directions.add(node1); directions.add(node2);
	 * 
	 * GameSession gameSession =
	 * GameSessionManager.getInstance().createNewGameSession(UUID.randomUUID().
	 * toString(), UUID.randomUUID(), "HostPlayer"); Player player = new
	 * Player(gameSession.getSessionName(), "Hans", UUID.randomUUID(), true);
	 * 
	 * Map map = gameSession.getMap(); Square squareLoco =
	 * map.getSquare(squarePosXLoco, squarePosYLoco); Square squareCart =
	 * map.getSquare(squarePosXCart, squarePosYCart); Square squareNext =
	 * map.getSquare(squarePosXNext, squarePosYNext);
	 * 
	 * Rail railLoco = new Rail(gameSession.getSessionName(), squareLoco,
	 * directions); Rail railCart = new Rail(gameSession.getSessionName(),
	 * squareCart, directions); Rail railNext = new
	 * Rail(gameSession.getSessionName(), squareNext, directions);
	 * 
	 * squareLoco.setPlaceable(railLoco); squareCart.setPlaceable(railCart);
	 * squareNext.setPlaceable(railNext);
	 * 
	 * Loco loco = new Loco(gameSession.getSessionName(), squareLoco, map,
	 * player.getId());
	 * 
	 * MessageQueueStub messageQueueStub = new MessageQueueStub();
	 * loco.addObserver(messageQueueStub);
	 * 
	 * // Methode, die in diesem Fall getestet wird loco.drive();
	 * 
	...
	public void LocoFindsRailItStandsOn() {
		...
		Rail rail = new Rail(gameSession.getDescription(), square, directions);
	...
	private Loco createTestLoco() {
		...
		PlayerLoco loco = new PlayerLoco(gameSession.getDescription(), squareLoco, player.getId());
